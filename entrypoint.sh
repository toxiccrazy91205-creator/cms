...
if ! su - postgres -c "psql -lqt | cut -d \| -f 1 | grep -qw cmsdb"; then
    ...
    su - postgres -c "psql -c \"ALTER USER cmsuser WITH SUPERUSER;\""
    su - postgres -c "psql -c \"ALTER USER postgres WITH PASSWORD 'cms';\""
...
su - cmsuser -c "/home/cmsuser/cms/bin/cmsWebServer" &
